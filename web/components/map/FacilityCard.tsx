...
export function FacilityCard({
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-md px-3 py-2">
      ...
      {/* 2행: 기준·접수·계획 + 여유량 */}
      <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-1 text-[12px]">
        ...
        <span
          className={`whitespace-nowrap ml-auto text-[12px] font-bold px-1.5 py-0.5 rounded flex-shrink-0 ${
  ...
  return (
    <div className="bg-gray-50 rounded px-2 py-1 flex items-center justify-between">
      ...
      <span className="font-semibold text-gray-900 tabular-nums">
        {cnt ?? 0}건
        {pwr != null && pwr > 0 && (
          <span className="text-[11px] text-gray-400 font-normal ml-1">({formatPower(pwr)})</span>
        )}
      </span>
    </div>
  );
