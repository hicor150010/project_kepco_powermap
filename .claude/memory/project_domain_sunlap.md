---
name: 도메인 sunlap.kr 연결 작업 중
description: 가비아 sunlap.kr → Vercel 연결. Vercel 측 도메인 추가 완료 (2026-04-21). 가비아 DNS 입력 남음
type: project
---
...
- ✅ Vercel 프로젝트(`kepco-powermap`) 에 `sunlap.kr` + `www.sunlap.kr` 추가
- ✅ Vercel 자동 구성: `sunlap.kr` → `www.sunlap.kr` 로 307 리다이렉트, `www` 가 메인
...
- 전파 확인: `nslookup sunlap.kr` 또는 https://dnschecker.org
- Vercel Domains 화면에서 Refresh 클릭 → Valid Configuration 되면 완료
- SSL 인증서 자동 발급 (Let's Encrypt, 수 분 내)
- 연결 후 Kakao Developers 콘솔에 `https://sunlap.kr`, `https://www.sunlap.kr` 추가 필수 (카카오맵 SDK 차단 방지)